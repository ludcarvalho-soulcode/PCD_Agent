...
async def _raspar_todas_paginas(page, fonte: dict, max_paginas: int, log: Callable) -> list[dict]:
    ...
    for tentativa in range(3):
        try:
            ...
            if tentativa == 2:
                # Estes comandos não apagam seu raise, eles apenas mostram o erro antes de subir a falha
                print(f"ERRO DEFINITIVO NA TENTATIVA {tentativa+1}: {str(e)}")
                import traceback
                traceback.print_exc()
                
                # O raise continua aqui, garantindo que o fluxo de erro original seja mantido
                raise 
...
async def raspar_tacs_pcd(
    ...
    async with async_playwright() as pw:
        # Aumentamos o timeout de lançamento do navegador para 60s
        browser = await pw.chromium.launch(
            headless=True,
            args=["--no-sandbox", "--disable-setuid-sandbox", "--disable-dev-shm-usage"],
        )
        
        # Criamos o contexto já definindo o timezone para não causar suspeitas
